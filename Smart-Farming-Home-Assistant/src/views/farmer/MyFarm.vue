...
const farmLocation = computed(() => {
  const { farmProvince, farmCity, farmDistrict, farmAddress } = userInfo.value
  const parts = [farmProvince, farmCity, farmDistrict].filter(Boolean)
  return parts.join('') + (farmAddress ? ' ' + farmAddress : '')
})

const cropNames = computed(() => crops.value.map(c => c.name))

const cropList = computed(() => {
  const randomStatuses = ['生长中', '成熟期', '种植期', '收获期']
...
<template>
  <div class="page-container">
    ...
    <main class="main-content">
      ...
      <div class="farm-card">
        <h2>{{ userInfo.username || '我的农场' }}</h2>
        <div class="farm-details">
          <div class="detail-item">
            <span class="detail-icon">📍</span>
            <span>{{ farmLocation || '暂无地址信息' }}</span>
          </div>
          <div class="detail-item">
            <span class="detail-icon">🌾</span>
            <span>作物: {{ crops.length }}种</span>
          </div>
          <div class="detail-item">
            <span class="detail-icon">📧</span>
          ...
          <div class="detail-item">
            <span class="detail-icon">📱</span>
            <span>{{ userInfo.phone || '暂无电话' }}</span>
          </div>
        </div>
        <div class="crops-tag">
          <span v-for="crop in cropNames" :key="crop" class="crop-tag">
            {{ crop }}
          </span>
        </div>
      </div>
